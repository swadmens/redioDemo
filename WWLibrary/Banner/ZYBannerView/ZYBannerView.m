...
{
    UICollectionViewCell *getCell;
    if ([self.dataSource respondsToSelector:@selector(bannerCollectionView:cellForItemAtIndexPath:)]) {
        getCell = [self.dataSource bannerCollectionView:collectionView cellForItemAtIndexPath:[NSIndexPath indexPathForItem:(indexPath.item % self.itemCount) inSection:indexPath.section]];
        if (getCell) {
            return getCell;
        }
    }
...
- (void)collectionView:(UICollectionView *)collectionView didEndDisplayingCell:(UICollectionViewCell *)cell forItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    NSInteger currentPage = currentIndexPath.item % self.itemCount;
    self.pageControl.currentPage = currentPage;
    if ([self.delegate respondsToSelector:@selector(banner:didScrollsToPage:)]) {
        [self.delegate banner:self didScrollsToPage:currentPage];
    }
}
...
{
    if ([self.delegate respondsToSelector:@selector(banner:didScroll:)]) {
        [self.delegate banner:self didScroll:scrollView];
    }
...
- (UICollectionView *)collectionView
{
    if (!_collectionView) {
        ...
        _collectionView.alwaysBounceHorizontal = NO; // 小于等于一页时, 允许bounce
