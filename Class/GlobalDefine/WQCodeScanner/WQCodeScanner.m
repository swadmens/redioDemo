    ...
    [AVCaptureDevice requestAccessForMediaType:AVMediaTypeVideo completionHandler:^(BOOL granted) {
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            if (granted) {
                ...
                NSString *title = _L(@"Please Allow App to access your camera in the settings of iPhone - Privacy - camera option");
                UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:title message:@"" delegate:nil cancelButtonTitle:_L(@"OK") otherButtonTitles:nil];
                [alertView show];
            }
...
- (void)viewWillAppear:(BOOL)animated {
    ...
    NSString *codeStr = @"";
    switch (_scanType) {
        case WQCodeScannerTypeAll: codeStr = _L(@"QR Code／Bar Code"); break;
        case WQCodeScannerTypeQRCode: codeStr = _L(@"QR Code"); break;
        case WQCodeScannerTypeBarcode: codeStr = _L(@"Bar Code"); break;
        default: break;
    }
    ...
    if (self.tipStr && self.tipStr.length > 0) {
        ...
        self.tipLabel.text= [NSString stringWithFormat:_L(@"please put %@ into the frame"), codeStr];
    }
...
- (void)captureOutput:(AVCaptureOutput *)captureOutput didOutputMetadataObjects:(NSArray *)metadataObjects fromConnection:(AVCaptureConnection *)connection {
    ...
    if (metadataObjects.count > 0) {
        ...
        if ([result containsString:@"SN:"]) {
            NSArray *arr1 = [result componentsSeparatedByString:@"SN:"];
